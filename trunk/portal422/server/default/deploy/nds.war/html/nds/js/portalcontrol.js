...
PortalControl.prototype = {
    initialize: function () {
        ...
        try {
            ...
            $("portal_middle_left_fmenu").innerHTML = "<a href='#' style='' class='portal-icon arrow-normal' title='' onclick='pc.menu_toggle(this);'></a>";
	...
	*/
    resize: function () {
        ...
        $("portal-content").style.width = "100%";
        ////var e=$("embed-lines");
        ////识别IE8 列表宽度问题
        //var leftmenu = jQuery("#portal_middle_left_fmenu");
        //var leftmenuWidth = leftmenu.length>0?leftmenu.width():0;
        //if(navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.match(/8./i)=="8."){
        //    limitWidth = (jQuery(".nav_right").css("display") == "block") ? jQuery(".nav_right").width() + jQuery("#portal_middle_left_fmenu").width() :leftmenuWidth;
        //}
	...
	*/
    ...
    _updateGridLine: function (row) {
        ...
        if (err_omsg && line[2] != null) {
            err_omsg.poshytip({
                className: 'tip-green', alignX: 'inner-left',
    ...
    navigate: function (tn, tgt) {
        ...
        new Ajax.Request(url, {
            ...
            onSuccess: function (transport) {
                ...
                commjs_registCustomerScrollBar(jQuery(pt));
	...
	*/
    ...
    _submitToNewWindow: function (resulthandler, width, height) {
        //这里为了解决url长度问题，所以导出类似操作都使用post提交，
        //但是由于提交只能提交至新的浏览器窗口，体验效果不好，所以这里还是使用模态窗口
        //只不过是吧form的target设置到模态窗口的iframe标签上，从而实现无刷新提交，并且窗口使用的本页面的模态窗口
        //从而提高体验度
        var fm = $("export_form");
        $("exp_resulthandler").value = resulthandler;
        $("query_json").value = Object.toJSON(this._gridQuery);
        var modal = showObject3('', width, height, {});
        var iframe = modal.DOM.content.find("iframe");
        var name = iframe.attr("name") || "";
        if (name == "") {
            name = "________export_form_iframe"
            iframe.attr("name", name);
        }
        fm.setAttribute("target", name);
        fm.submit();
    },
	...
	*/
    ...
    doExportList: function () {
        this._submitToNewWindow("/html/nds/reports/create_report.jsp", 500, 190);
    },
    /**
                ...
                showObject2("/html/nds/cxtab/userdims.jsp?id=" + cxtabId,
    				{
    				    ...
    				    maxButton: false, closeButton: true
    				}, 700, 480);
                return;
            }
        }
...
function showObject3(url, theWidth, theHeight, option) {
    if (theWidth == undefined || theWidth == null) theWidth = 956;
    if (theHeight == undefined || theHeight == null) theHeight = 570;
    //var options={width:theWidth,height:theHeight,title:gMessageHolder.IFRAME_TITLE, modal:true,centerMode:"x",noCenter:true,maxButton:true};
    var options = $H({ width: theWidth, height: theHeight, title: gMessageHolder.IFRAME_TITLE, skin: 'aero', ifrid: 'popup-iframe-0', drag: true, resize: true, lock: true, esc: true, skin: 'chrome', ispop: true });
...
mufavorite.prototype = {
    ...
    add_mufavorite: function (tb_name, tb_id, rpt, cx_id) {
        ...
        art.dialog.confirm('是否添加到我的收藏夹?', function (topWin) {
            ...
            portalClient.sendRequest(a, function (response) {
                ...
                if (p_id > 0) {
                    art.dialog.tips('[' + tb_name + ']收藏夹中已存在,无需添加!', null, 'warning');
                    return;
                }
			...
			function () {
			    ...
			    portalClient.sendRequest(a, function (response) {
			        ...
			        for (var i = 0; i < rows.length; i++) {
			            p_id = rows[i][0];
			            self.sendDeleteObject(p_id, values, i);
			        }
			    });
			},
			function () {
    ...
    sendDeleteObject: function (p_id, values, i) {
    ...
    flash_mufavorite: function (tb_id) {
        jQuery("#mu_favorite input[value=" + tb_id + "]").parent().hide('drop');
    },
    /**
