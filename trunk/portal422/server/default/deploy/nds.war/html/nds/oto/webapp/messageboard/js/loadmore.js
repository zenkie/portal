 $(document).ready(function(){
		...
		$(window).scroll(function(){
			var bounce = jQuery(".bounceInDown");
			var lastCh = bounce[bounce.length - 2];
			var lastid = jQuery(lastCh).find(".hfinfo").attr("date");
            var pageSize = 15;
            var total = 916;
            ...
            if(!state && (($(document).height()-range) <= totalheight) ) {
            	...
            	state = 1;				
				jQuery.ajax({
					url: '/html/nds/oto/webapp/messageboard/load.vml?lastid='+lastid,
					type: 'post',
