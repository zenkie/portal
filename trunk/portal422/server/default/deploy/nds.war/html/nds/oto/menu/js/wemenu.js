...
wemenu.prototype={
	initialize:function(){
		...
		//jQuery("ssssssssssssss").live("mousedown",function(e){
			...
			//jQuery("#"+node.parentNode.id+"-cont").bind("mousemove",function(e){
			jQuery("#initmenu").bind("mousemove",function(e){
				...
				jQuery("#initmenu div").not("[id$=-cont]").not(_this).not(wid).each(function(i) {
					var obj = jQuery(this);
			...
			jQuery(_this).bind("mouseup",function() {
				jQuery(_this).unbind("mouseup");
				jQuery(_this).unbind("mousemove");
				jQuery("#"+node.parentNode.id+"-cont").unbind("mouseup");
				jQuery("#"+node.parentNode.id+"-cont").unbind("mousemove");

				jQuery("#initmenu").unbind("mouseup");
				jQuery("#initmenu").unbind("mousemove");
	...
	initMenu:function(){
		webFXTreeConfig.rootIcon		= "/html/nds/oto/themes/01/images/root.gif";
		webFXTreeConfig.openRootIcon	= "/html/nds/oto/themes/01/images/root.gif";
		webFXTreeConfig.folderIcon		= "/html/nds/oto/themes/01/images/folder.png";
		webFXTreeConfig.openFolderIcon	= "/html/nds/oto/themes/01/images/openfolder.png";
		webFXTreeConfig.fileIcon		= "/html/nds/oto/themes/01/images/file.png";
		
		webFXTreeConfig.lMinusIcon		= "/html/nds/oto/themes/01/images/Lminus.png";
		webFXTreeConfig.lPlusIcon		= "/html/nds/oto/themes/01/images/Lplus.png";
		webFXTreeConfig.tMinusIcon		= "/html/nds/oto/themes/01/images/Tminus.png";
		webFXTreeConfig.tPlusIcon		= "/html/nds/oto/themes/01/images/Tplus.png";
		webFXTreeConfig.iIcon			= "/html/nds/oto/themes/01/images/I.png";
		webFXTreeConfig.lIcon			= "/html/nds/oto/themes/01/images/L.png";
		webFXTreeConfig.tIcon			= "/html/nds/oto/themes/01/images/T.png";
		webFXTreeConfig.blankIcon       = "/html/nds/oto/themes/01/images/blank.png";
		webFXTreeHandler.maxChild        =[3,5];
		//webFXTreeHandler.onSelect=function(item){return wem.editMenu(item);}
		//保存获取过来的json数据
		var jsondata="";
		var wemenu=this;
		jQuery.ajax({
			url: '/html/nds/oto/menu/getmenu.jsp',
			data:{},
			dataType:"json",
			type: 'post',
			async: false,
		...
		var tree;// = new WebFXTree('自定义菜单列表',null,null,3);
		var treeFirNode;
		//循环解析json对象
		var button;
		var action;
		var cbutton;
		tree = new WebFXTree('自定义菜单列表');
		if(jsondata&&jsondata.hasOwnProperty("button")){//{return;}
			webFXTreeHandler.nodeJson=jsondata;
			for(var i=0;i<jsondata["button"].length;i++){
				button=jsondata["button"][i];
				//action = "javascript:var options={width:963,height:399,title:'选择菜单',resize:false,drag:true,lock:true,esc:true,skin:'chrome',ispop:false,close:function(){}}; var url='/html/nds/oto/object/jumpUrl.jsp?fromid="+button.fromid+"&objid="+button.objid+"'; art.dialog.open(url,options);";
				//action="javascript:wem.editMenu(this,"+button.fromid+",'"+button.objid+"')";
				action="javascript:wem.editMenu(this);return false;";
				treeFirNode = new WebFXTreeItem(button.name,action,i);
				tree.add(treeFirNode);
				
				if(button.hasOwnProperty("sub_button")){
					for(var j=0;j<button["sub_button"].length;j++){
						cbutton=button["sub_button"][j];
						//action = "javascript:var options={width:963,height:399,title:'选择菜单',resize:false,drag:true,lock:true,esc:true,skin:'chrome',ispop:false,close:function(){}}; var url='/html/nds/oto/object/jumpUrl.jsp?fromid="+cbutton.fromid+"&objid="+cbutton.objid+"'; art.dialog.open(url,options);";
						//action="javascript:wem.editMenu(this,"+button.fromid+",'"+button.objid+"')";
						//action="javascript:webFXTreeHandler.focus(this);return false;";
						action="javascript:wem.editMenu(this);return false;";
						treeFirNode.add(new WebFXTreeItem(cbutton.name,action,j));
					}
				}
			}
		}
		...
		//var tree = new WebFXLoadTree("自定义菜单", "<%=NDS_PATH+"/oto/menu/getmenu.xml.jsp"%>");
		
		//var tree = new WebFXTree("自定义菜单", "<%=NDS_PATH+"/oto/menu/getmenu.xml.jsp"%>");
		//document.write(tree);
		jQuery("#initmenu").html(tree.toString());
	},
	//初始化所有菜单回复
