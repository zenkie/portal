...
function commjs_registCustomerScrollBar(s, opts) {
	return;//暂时不适用自定义滚动条
    var sbar = jQuery(s);
    if (sbar.perfectScrollbar) {
        sbar.perfectScrollbar('destroy');
        sbar.perfectScrollbar(opts);
        sbar.css({ overflowX: "hidden", overflowY: "hidden", "overflow": "hidden", "position": "relative" });
        if (sbar.is("body")) {
            sbar.css("position", "absolute");
        }
    }
}
...
function showGlobalMessage(message) {
    if (top.PortalTheme) {
        ...
        if (message.replace(/\s/g, '') != '') {
            top.PortalTheme.Tip.Info(target.html(), 2500);
        }
    }
}
...
(function () {
    ...
    function ieVersion() {
        if (_version == null) {
            ...
            var reg = /MSIE (\d+\.\d+);/;
            var s = (userAgent).match(reg);
            if (s == null) {
                reg = /rv:(\d+\.\d+)\)/;
                s = (userAgent).match(reg);
            }
            _version = s ? s[1] : -1;
        }
        return _version;
