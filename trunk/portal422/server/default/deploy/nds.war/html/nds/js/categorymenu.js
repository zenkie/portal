var commonCommands = { buttons: {}, menuItems: {}, mode: 'icon' }; //mode：icon 或者图片
/**
...
TableCommand.prototype.toButtonString = function () {
    ...
    if (this.img != null) {
        if (commonCommands.mode == "icon") {
            str += "<span class='iconfont " + this.img + "'></span>";
        } else {
            str += "<img src='/html/nds/images/" + this.img + "'>";
        }
    }
    str += this.desc + "</a>";
    return str;
...
(new TableCommand("Add", gMessageHolder.CMD_ADD, "icon-xinzeng1", "N")).makeTemplate();
(new TableCommand("Modify", gMessageHolder.CMD_MODIFY, "icon-xiugai1", "S")).makeTemplate();
(new TableCommand("Delete", gMessageHolder.CMD_DELETE, "icon-shanchu", "X")).makeTemplate();
(new TableCommand("Submit", gMessageHolder.CMD_SUBMIT, "icon-tijiaodingdan", "G")).makeTemplate();
(new TableCommand("Report", gMessageHolder.CMD_CXTAB, "icon-baogao", null)).makeTemplate();
(new TableCommand("ListAdd", gMessageHolder.CMD_LISTADD, "icon-xinzeng", null)).makeTemplate();
(new TableCommand("Import", gMessageHolder.CMD_IMPORT, "icon-daoru1", null)).makeTemplate();
(new TableCommand("ListCopyTo", gMessageHolder.CMD_LISTCOPYTO, "icon-fuzhi1", null)).makeTemplate();
(new TableCommand("UpdateSelection", gMessageHolder.CMD_UPDATE_SELECTION, "icon-iconfontcolor31", null)).makeTemplate();
(new TableCommand("UpdateResultSet", gMessageHolder.CMD_UPDATE_RESULTSET, "icon-zd_xgshuxing", null)).makeTemplate();
/*
废弃列表打印按钮
(new TableCommand("PrintList", gMessageHolder.CMD_PRINT_LIST,"tb_print.gif",null)).makeTemplate();
*/
(new TableCommand("ExportList", gMessageHolder.CMD_EXPORT_LIST, "icon-daochu", null)).makeTemplate();
(new TableCommand("SmsList", gMessageHolder.CMD_SMS_LIST, "icon-sms", null)).makeTemplate();
(new TableCommand("Refresh", gMessageHolder.CMD_REFRESH, "icon-shuaxin", "J")).makeTemplate();
(new TableCommand("Void", gMessageHolder.CMD_VOID, "icon-not_Selected", null)).makeTemplate();
(new TableCommand("Unvoid", gMessageHolder.CMD_UNVOID, "icon-danxuanxuanzhong", null)).makeTemplate();
(new TableCommand("PrintSelect", gMessageHolder.CMD_PRINT_SELECT, "icon-dayin4", null)).makeTemplate();
...
TableCommands.prototype.toString = function () {
    ...
    str += "</div>";
    if (this._menuItems.length > 1) {
        var imgOrIcon = "<img src='/html/nds/images/button_more.png'>";
        if (commonCommands.mode == "icon") {
            imgOrIcon = "<span class='iconfont icon-gengduo'></span>";
        }
        str += "<div class='portal-dock interactive-mode'><div class='table-buttons btn-more'><a href='#'>" + imgOrIcon +
...
CategoryTabItem.prototype.select = function () {
	...
	$("portal_middle_right_search_title").innerHTML="";
