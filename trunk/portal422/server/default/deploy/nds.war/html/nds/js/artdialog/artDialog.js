				  ...
				  <td <% if (icon) { %>colspan="2"<% } %> class="aui_td_title">\
					 <div class="aui_title_wrap">\
						<div id="<%=id%>title" class="aui_title">\
					  		<span class="aui_title_icon"></span><div id="pop-up-title-<%=queryindex%>"><%=title%></div>\
						</div>\
						  <% if (ispop) { %><a id="<%=id%>pop" class="aui_pop" href="#"><%=popText%></a><% } %>\
...
var dialog = function(data){
	...
	var ui = {
		...
		yesBtn: $(id + 'yes'),						// 确定按钮
		noBtn: $(id + 'no'),						// 取消按钮
		dcloseBtn: $(id + 'close'),					// 关闭按钮
		popBtn:$(id + 'pop'),  // popupwin
		...
		on: function(){
			...
			if (lockList.length === 1) {
				...
				data.effect && !isFilters ?
					$overlay.addClass('art_opacity').opacityFlash(0.5) :
	...
	var api = {
		...
		focus: function(elem){
			if (typeof elem === 'string') elem = $(elem)[0] || $('#' + elem)[0];
			elem = ($.isElem(elem) && elem) || ui.noBtn[0] || ui.yesBtn[0] || ui.dcloseBtn[0];
		...
		if (w ){
			...
			//alert(iframe0);
			if(iframe0){
				popup_window(iframe0.src,data.target, data.width,data.height);
				api.close();
			//alert(123);
			}
		}
	...
	ui.dialog.live('click', function(event){
		...
		switch (this) {
			...
			case ui.noBtn[0]:		// 取消按钮
				api.noFn();
				break;
			case ui.dcloseBtn[0]:	// 关闭按钮
				api.closeFn(event);
				break;
			case ui.popBtn[0]:	// pop按钮
				api.popBtn(event);
				break;
			default:				// 其他元素
...
$.fn.dialog.open = function(url, options){
	...
	var load, $iframe, iwin,
		...
		data = {
			window: 'top',
			content: {url: url},
			tmpl: '<iframe id="'+ifrid+'" class="' + id + '" src="<%=url%>" frameborder="0" allowtransparency="true"></iframe>',
