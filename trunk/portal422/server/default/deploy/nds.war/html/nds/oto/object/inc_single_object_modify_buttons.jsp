...
if( objectId == -1){
    if(canAdd){
 		validCommands.add( commandFactory.newButtonInstance("Create", 
			...
			"oc.doCreate()",null,"/html/nds/oto/themes/01/images/tb_new.gif"
		));
    }
}else{
	if(canModify){
		validCommands.add( commandFactory.newButtonInstance("Modify", 
			...
			"oc.doModify()",null,"/html/nds/oto/themes/01/images/tb_modify.gif"
		));
	}
	if(canDelete){
	...
	if(canVoid){
		validCommands.add( commandFactory.newButtonInstance("Void", 
			...
			"oc.doVoid()",null,"/html/nds/oto/themes/01/images/tb_void.gif"
		));
	}
	if(canSubmit){
		...
		validCommands.add( commandFactory.newButtonInstance("Submit", 
			...
			"oc.doSubmit("+ canModify +","+shouldWarn+")",null,"/html/nds/oto/themes/01/images/tb_submit.gif"
		));
	...
	validCommands.add( commandFactory.newButtonInstance("Print", 
			...
			"oc.doPrint()",null,"/html/nds/oto/themes/01/images/tb_print.gif"
			));	
