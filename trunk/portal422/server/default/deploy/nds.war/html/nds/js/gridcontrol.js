    ...
    saveLine: function () {
        ...
        if (chkMaterialAttribute && -1 != this._getPositionInData("Y_MATERIALALIAS_ID__NO")) {
            ...
            if (chkMaterialAttribute != null && chkMaterialAttribute.checked && !mtlValue.blank()) {
				...
				evt.masterTable=oc._masterObj.table.id;
    ...
    _checkProductAttribute: function (e) {
        ...
        if (chkResult.code != 0) {
            ...
            if (chkResult.showDialog) {
				...
				executeLoadedScript(ele);
				*/
                var options = $H({ id: "art_itemdetail_div", opacity: 0.1, title: gMessageHolder.SET_PRODUCT_ATTRIBUTE, padding: 0, resize: true, drag: true, lock: true, esc: true, skin: 'chrome' });
	...
	*/
    _refreshGrid: function (e) {
		...
		*/
        ...
        if ($("modify_table")) {
            setTimeout(function () {
                ScrollableTable.set('modify_table', '100%', 275, false);
            }, 20);
        }
        var container = jQuery("#embed-items");
        container.unbind('scroll').bind('scroll', function () {
            var target = jQuery(this);
            var ml = this.scrollWidth - this.clientWidth - this.scrollLeft;
            ml = ml < 0 ? 0 : ml;
            target.find("#D_modify_table_B .ps-scrollbar-y-rail").css("margin-right", ml);
        });
        //记住该代码不要放到 container.unbind('scroll')之前，不然自定义滚动条实现机制就被打破了
        //因为自定义滚动条底层有使用到scroll事件 如果取消掉所有scroll事件会导致自定义滚动条出现问题
        commjs_registCustomerScrollBar("#embed-items", { suppressScrollY: true, wheelPropagation: true });
        commjs_registCustomerScrollBar("#D_modify_table_B", { suppressScrollX: true, wheelPropagation: true });
	...
	*/
    ...
    _initTable: function () {
		...
		*/
        ...
        tb.unbind("dblclick").dblclick(function (trElement) {
