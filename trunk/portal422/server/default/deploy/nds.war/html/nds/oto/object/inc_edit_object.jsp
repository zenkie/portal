...
<table align="center" border="0" cellpadding="1" cellspacing="1" width="100%" class="emtb">
			...
			if(values != null){
			   ...
			   Iterator temp = values.keys();
				...
				if(refTable !=null){%>
					...
					%>
						<input:text name="<%=columnDomName%>" attributes="<%= h %>" default="<%=defaultValue%>"  attributesText="<%=fixedColumnMark%>" /><%= typeIndicator%>
						<span id="att_<%=columnDomId%>"  onclick="gc.showUploadDlg(<%=column.getId()%>,'<%=columnDomId%>')"><img border=0 width=16 height=16 align=absmiddle src='/html/nds/oto/themes/01/images/attach.gif' title='<%= PortletUtils.getMessage(pageContext, "open-new-page-to-config" ,null)%>'></span>            	
						<script>createButton(document.getElementById("att_<%=columnDomId%>"));</script> 	
					<%}else if( column.getDisplaySetting().getObjectType()==DisplaySetting.OBJ_XML){
						//h.put("class",column.getDisplaySetting().getRows()>1? TableQueryModel.getTextAreaInputCssClass(columnsPerRow,column):TableQueryModel.getTextInputCssClass(columnsPerRow,column));
						//String filterXml=(dataDB==null?"":dataDB.toString());
						String filterXml="";
						nds.schema.Filter fo=new nds.schema.Filter(filterXml);
						%>
						<input:filter id="<%=columnDomId%>" columnId="<%=String.valueOf(column.getId())%>" desc="<%=fo.getDescription()%>" xml="<%=filterXml%>" name="<%=columnDomName%>" attributes="<%=h%>" attributesText=""/>
					<%}else{%>
						<input:text name="<%=columnDomName%>" attributes="<%= h %>" default="<%=defaultValue%>"  attributesText="<%=fixedColumnMark%>" /><%= typeIndicator%>
						&nbsp;&nbsp;&nbsp;
					<%}
	...
	int checkProductAttribute= Tools.getInt(((Configurations)WebUtils.getServletContextManager().getActor(nds.util.WebKeys.CONFIGURATIONS)).getProperty("item.product.checkattribute", "1"), 1);
