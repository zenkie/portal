...
keywords.prototype={
	...
	onreadyHtml:function(){
			...
			jQuery('#txtReplyWords').bind({ 'mouseup' : function getJiaodian(v){
				...
				jQuery("#spnCnt").text(notFoundKey.mbStringLength());
				...
				jQuery.post("/html/nds/oto/attention/getTuwenData.jsp",
						...
						function(tudata){
							...
							jQuery.each(keySort,function(index,val){
								...
								s += '            <a href="javascript:void(0);" onclick="notFoundKey.editTuwen(this);" class="th icon18 iconEdit">编辑</a>';
								s += '        </li>';
								s += '        <li class="b-dib sub-msg-opr-item">';
	...
	commitSave:function(){
		...
		evt.action = "aud";
		evt.permission="r";
		notFoundKey._executeCommandEvent(evt);
		
	},
	_commitSave:function(e){
		tuwen={};
		art.dialog.tips("操作成功");
		jQuery(".tag-panel-head a:last").click();
		//art.dialog.close();
	},
	...
	AddSubscribeMark:function(){
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				notFoundKey.insertText("#txtReplyWords","[@a href=\"weixin://contacts/profile/"+eval("("+data+")")[0].rows+"\"@]点击关注[@/a@]");
	...
	AddMenuMark:function(){
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				notFoundKey.insertText("#txtReplyWords",s);
				//notFoundKey.setRemainString();
			}
		});
		...
		var options={
				...
				close:function(){
					...
					if(typeof(url)!="undefined"){
						notFoundKey.insertText("#txtReplyWords",url);
						//notFoundKey.setRemainString();
					}
				}
			}; 
	...
	insertText:function(textarea, str){
		if(jQuery.isEmptyObject(range)){
			...
			jQuery("#spnCnt").text(notFoundKey.mbStringLength(notFoundKey.mbStringLength()));
			return;
		}
		...
		jQuery("#spnCnt").text(notFoundKey.mbStringLength(jQuery("#txtReplyWords").val()));
