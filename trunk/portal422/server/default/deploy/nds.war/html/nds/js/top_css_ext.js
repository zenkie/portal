...
function btn_dialog_window(url, warning) {
    if (warning != undefined) {
        ...
        art.dialog.confirm(warning, function () {
            dialog_window(url, 'auto', 'auto', true);
        },
        function () {
            //alert('你取消了操作!');
        });
...
function dialog_window(url, theWidth, theHeight, refreshWindowWhenClose) {
    ...
    top.PortalTheme.ShowLoading(working);
    var evt = url.toQueryParams();
    Controller.handle(Object.toJSON(evt), function (r) {
        try {
            top.PortalTheme.CloseLoading();
...
function popup_window(url, tgt, theWidth, theHeight) {
    ...
    if (!isDownloadURL(url)) {
        showDialog(url, theWidth, theHeight);
    }
    //newWindow.focus();
}
//判断指定路径是否为下载文件路径
function isDownloadURL(url) {
    url = url || "";
    var downloadFileUrls = ['/servlets/binserv/getfile', '/servlets/binserv/download', '/blhandler/export/'];
    var pathname = url.substring(0, url.indexOf("?")).toLowerCase();
    var isDurl = false;
    for (var i = 0, k = downloadFileUrls.length; i < k; i++) {
        if (pathname.indexOf(downloadFileUrls[i]) > -1) {
            isDurl = true;
            break;
        }
    }
...
//静默模式下载文件
function silentDownload(filePath, callBack) {
    var downloader = document.getElementById("downloadHref");
    downloader = downloader || document.createElement("iframe");
    downloader.id = "downloadHref";
    downloader.style.display = "none";
    downloader.src = filePath;
    document.body.appendChild(downloader);

    var isInvoke = false;
    var lisInter = setInterval(downloadListener, 500);
    function downloadListener() {
        ...
        if (state == "complete" || state == "loaded") {
            ...
            var dd = document.getElementById("downloadHref");
            ...
            if (typeof dd.contentWindow.document != 'unknown') {
                if (dd.contentWindow.document && dd.contentWindow.document.body) {
                    var s = dd.contentWindow.document.body.innerHTML;
                    s = s || '';
                    if (s != "") {
                        art.dialog({ content: s, width: 300, title: '文件下载异常', height: 150, animation: { inCss: 'translateFromTop', outCss: 'translateToBottom' } });
                    }
                }
            }
        }
