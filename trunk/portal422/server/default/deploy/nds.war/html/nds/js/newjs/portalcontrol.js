...
PortalControl.prototype = {
	initialize: function() {
		...
		try{
			...
			$("portal_middle_left_fmenu").innerHTML="<a href='#' style='opacity:0.4' class='icon0-1' title='返回'></a>"+tabs.toString();
			//alert(tabs);
			//查询功能按钮
			//识别IE8 没有查询功能
...
/**
	...
	*/
	_refreshGrid :function (e) {
		...
		var tHead=jQuery("#div_head #table_head thead tr:first td");
		var tFoot=jQuery("#div_foot #table_foot tfoot tr:last td");
		jQuery("#embed-lines #inc_table thead tr:first td").each(function(i){
			jQuery(this).css({"width":"","min-width":""});
			if(tHead){jQuery(tHead[i]).css({"width":"","min-width":""});}
			if(tFoot){jQuery(tFoot[i]).css({"width":"","min-width":""});}
		});
			...
			if(qr.pagecontent!=null){
				...
				if(Prototype.Browser.IE){
					...
					*/
					...
					/*//paco add begin
					var thead=jQuery("#inc_table").find("thead");
					jQuery(thead).removeAttr("style");
					//paco add end
					*/
		...
		jQuery('#grid_table').find("tr").each(function(j){
			jQuery(this).attr("height","24");
		});
		...
		/*paco add begin */
		var div_thead=jQuery("#div_head");
		var thead=jQuery("#inc_table").find("thead");				
		if(div_thead.size()==0){
			//this._scrollTable();
			var options={};
			options.isScrollFoot=true;
			options.isAddParentDiv=true;
			options.parentDiv="<div style='margin-left:5px;'></div>";
			options.footLocation="last";
			options.table_body="#inc_table";
			options.div_body="#embed-lines";
			options.table_head="<table id='table_head' cellspacing=1 border=0 class='my_thead' style='width:100%;border-collapse:separate;'></table>";
			options.table_foot="<table id='table_foot' cellspacing=1 border =0 style='background-color:rgb(194, 180, 180);border-collapse:separate;'></table>";
			options.attributes={"cellspacing":"1"};
			options.styles={"background-color":"#DFDFDF","border-collapse":"separate"};
			
			this._setScrollTableBody(options);
			/*
			var width=0;
			var theadTds=thead.find("tr:first td");
			var tbodyTds=jQuery("#inc_table").find("tbody tr:first td");
			if(tbodyTds && theadTds){
				tbodyTds.each(function(index){
					width=this.offsetWidth;
					//width=jQuery(this).width();
					// if(index==tbodyTds.size()-1){jQuery(theadTds[index]).attr("style","min-width:"+(width+_getScrollbarWidth())+"px");}
					// else{jQuery(theadTds[index]).attr("style","min-width:"+width+"px");}
					jQuery(theadTds[index]).attr("style","min-width:"+width+"px");
					jQuery(this).attr("style","min-width:"+width+"px");
				});
			}
			jQuery(thead).attr("style","position:absolute;left:0px;top:0px;");
			var div_scroll=$("embed-lines");
			div_scroll.onscroll=function(){
				jQuery(thead).css({"left":"-"+this.scrollLeft+"px"});
			}
			
			jQuery("#embed-lines #inc_table thead tr:first td").bind('resize',function(e){
				alert("td resize");
			});
			*/
		}
		...
		/*
		//jQuery("#inc_table thead tr:first td").bind('resize',function(e){
			//alert(e.target.innerText);
			// var thead=jQuery("#div_head #table_head thead tr:first td");
			// jQuery("#embed-lines #inc_table thead tr:first td").each(function(i){
				// jQuery(this).css({"width":jQuery(this).width()+"px"});
				// jQuery(thead[i]).css({"width":(jQuery(this).width()+2)+"px"});
			// });
		//});
		// jQuery("#inc_table thead tr:first td").bind('resize',function(e){
			// alert(e.target.innerText);
		...
		*/
		...
		setTimeout(function(){
			...
			jQuery("#embed-lines #inc_table thead tr:first td").each(function(i){
				if(!colspan){
					colspan=jQuery(tfoot[i]).attr("colspan");
					cspan=parseInt(colspan,0);
					if(cspan){allCSpan+=cspan;}
				}
				
				sourceWidth=jQuery(this).width();
				targetWidth=jQuery(thead[i]).width();
				maxWidth=Math.max(sourceWidth,targetWidth);
				
				if(colspan){
					currentCol++;
					allWidth+=maxWidth;
					if(cspan==currentCol){
						tFootWidth=jQuery(tfoot[i-allCSpan+1]).width();
						if(tFootWidth>allWidth){
							maxWidth=tFootWidth-allWidth+maxWidth;
							allWidth=tFootWidth;
							jQuery(this).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
							jQuery(thead[i]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
							jQuery(tfoot[i-allCSpan+1]).css({"width":(allWidth+1*cspan-1)+"px","min-width":(allWidth+1*cspan-1)+"px"});
						}
						else{
							jQuery(this).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
							jQuery(thead[i]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
					...
					tFootWidth=jQuery(tfoot[i-allCSpan+1]).width();
					maxWidth=Math.max(sourceWidth,targetWidth,tFootWidth);
					jQuery(this).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
					jQuery(thead[i]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
					jQuery(tfoot[i-allCSpan+1]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
				}
				
				/*
				if(targetWidth>sourceWidth){
					jQuery(this).css({"min-width":targetWidth+"px"});
					jQuery(thead[i]).css({"min-width":targetWidth+"px"});
				}
				else{
					jQuery(this).css({"min-width":sourceWidth+"px"});
					jQuery(thead[i]).css({"min-width":sourceWidth+"px"});
				}
				*/
			});
			},
		...
		//设置高度
		var maxHeight=0;
		var divBody=jQuery("#embed-lines");
		var allHeight=jQuery("#portal_middle_right").height();
		var th=jQuery("#portal_middle_right_search").height();
		var topHeight=jQuery("#page-table-query").height();
		var middleHeight=jQuery("#query_buttom").height();
		var bo=jQuery("#result-filter-desc").height();
		var bs=jQuery("#list-legend").height();
		var dh=jQuery("#div_head").height();
		var df=jQuery("#div_foot").height();
		divBody.css({"max-height":(allHeight-th-topHeight-middleHeight-bo-bs-dh-df-25)+"px"});
	},
	
	_scrollTable:function(){
		var width=0;
		var divBody=jQuery("#embed-lines");
		var divScroll=jQuery(divBody).wrap("<div style='margin-left:5px;'></div>");
		var table=jQuery("#inc_table");
		jQuery(table).css({"margin-top":"-27px","margin-buttom":"-40px"});
		jQuery(table).css({"background-color":"#DFDFDF","border-collapse":"separate","white-space":"nowrap"});
		jQuery(table).attr({"cellspacing":"1"});
		var tableThead=jQuery(table).find("thead");
		var tableFoot=jQuery(table).find("tfoot");
		/*jQuery('tr:first td',tableThead).each(function(index){
			width=this.offsetWidth;
			jQuery(this).css({"min-width":width+"px"});//.css({"min-width":width+"px","white-space":'nowrap'});
		});
		jQuery('tr:last td',tableFoot).each(function(index){
			width=this.offsetWidth;
			jQuery(this).css({"min-width":width+"px"});//.css({"min-width":width+"px","white-space":'nowrap'});
		});
		*/
		var tThead=tableThead.clone(true);
		//var tFoot=tableFoot.clone(true);
		//jQuery(tableFoot).css({"visibility":"hiddle"});
		jQuery(tThead).attr("class","my_thead");
		var divHead=jQuery("<table id='table_head' cellspacing=1 border=0 style='width:100%;background-color:#DFDFDF;border-collapse:separate;background:#494944!important;white-space:nowrap;'></table>").insertBefore(divBody).append(tThead).wrap("<div id='div_head' style='overflow:hidden;width:100%;'></div>");
		var divFoot=jQuery("<table id='table_foot' cellspacing=1 border =0 style='background-color:rgb(194, 180, 180);border-collapse:separate;white-space:nowrap;'></table>").insertAfter(divBody).append(tableFoot).wrap("<div id='div_foot' style='overflow:hidden;width:100%;'></div>");
		
		var div_scroll=$("embed-lines");
		//jQuery(div_scroll).wrap("<div></div>");
		div_scroll.onscroll=function(){
			jQuery('#table_head').css({"margin-left":"-"+this.scrollLeft+"px"});
			jQuery('#table_foot').css({"margin-left":"-"+this.scrollLeft+"px"});
		}
		// jQuery("#inc_table thead tr:first td").bind('resize',function(e){
			// alert(e.target.innerText);
		// });
	...
	/**paco add scroll begin
	 *div_body:滚动内容的外层IDV
	 *table_body:滚动内容
	 *isAddParentDiv:是否添加滚动内容的最外层DIV
	 *isScrollFoot:是否滚动FOOT
	 *if(isAddParentDiv){<div id="parent_div">}
	 *	<div id="div_head"><table id="table_head"></table></div>
	 *	<div id="div_body"><table id="table_body"></table></div>
	 *	if(isScrollFoot){<div id="div_foot"><table id="table_foot"></div>}
	 *if(isAddParentDiv){</div>}
	 */
	_setScrollTableBody:function(options){//div_head,table_head,div_body,table_body,div_foot,table_foot,footLocation,isAddParentDiv,isScrollFoot){
		var isScrollFoot=options.isScrollFoot;
		var isAddParentDiv=options.isAddParentDiv;
		var parent_div=options.parentDiv;
		var footLocation=options.footLocation; //取值为:first/last。如果foot滚动时，设置列宽度的列是第一行还是最后一行，因为有的foot第一行或最后一行可能是隐藏的，设置宽度无效。
		var table_head=options.table_head;
		var div_head=options.div_head;
		var table_body=options.table_body;
		var div_body=options.div_body;
		var table_body_attribute=options.attributes;
		var table_body_style=options.styles;
		var table_foot=options.table_foot;
		var div_foot=options.div_foot;
		
		var parentDiv;
		if(isAddParentDiv){parentDiv=parent_div?parent_div:"<div></div>";}
		var table=jQuery(table_body);
		var tableThead=jQuery(table).find("thead"); 
		var bodyStyles={"margin-top":"-"+(jQuery(tableThead).height()+2)+"px"}
		if(table_body_style){bodyStyles=jQuery.extend(bodyStyles,table_body_style);}
		jQuery(table_body).css(bodyStyles);
		if(table_body_attribute){jQuery(table_body).attr(table_body_attribute);}
		var tableTfoot=null;
		if(isScrollFoot){tableTfoot=jQuery(table).find("tfoot");}
		
		var dHead=div_head?div_body:"<div id='div_head' style='overflow:hidden;width:100%;'></div>";
		var tHead=table_head?table_head:"<table id='table_head' cellspacing=1 border=0 style='width:100%;border-collapse:separate;'></table>";
		var dBody=div_body;
		if(!div_body || (div_body && !jQuery(div_body))){
			if(!div_body){dBody="<div id='div_body' style='overflow:hidden;width:100%;'></div>";}
			dBody=jQuery(table_body).wrap(dBody).parent();
			if(isAddParentDiv){jQuery(dBody).wrap(parentDiv);}
		}else{
			dBody=jQuery(div_body);
			if(isAddParentDiv){jQuery(dBody).wrap(parentDiv);}
		}
		var dFoot=div_foot?div_foot:"<div id='div_foot' style='overflow:hidden;width:100%;'></div>";
		var tFoot=table_foot?table_foot:"<table id='table_foot' cellspacing=1 border =0 style='border-collapse:separate;'></table>";
		
		var tThead=tableThead.clone(true);
		var tableHead=jQuery(tHead).insertBefore(dBody).append(tThead).wrap(dHead);
		var tableFoot;
		if(tableTfoot){tableFoot=jQuery(tFoot).insertAfter(dBody).append(tableTfoot).wrap(dFoot);}

		//设置滚动BODY时，表头与FOOT一起滚动
		dBody=jQuery(dBody);
		if(dBody.size()>0){
			dBody[0].onscroll=function(){
				jQuery('#table_head').css({"margin-left":"-"+this.scrollLeft+"px"});
				if(tableFoot){jQuery('#table_foot').css({"margin-left":"-"+this.scrollLeft+"px"});}
			}
		}
		
		//同步表头与FOOT的列宽与BODY的列宽一至
		...
		setTimeout(function(){
			...
			var targetWidth=0;
			var tFootWidth=0;
			var tHTDS=jQuery("thead tr:first td",tableHead);
			var tFTDS=null;
			if(footLocation=="first"){jQuery("tfoot tr:first td",tableFoot);}
			else{jQuery("tfoot tr:last td",tableFoot);}
			jQuery("#embed-lines #inc_table tHTDS tr:first td").each(function(i){
				if(isScrollFoot && !colspan){
					colspan=jQuery(tFTDS[i]).attr("colspan");
					cspan=parseInt(colspan,0);
					if(cspan){allCSpan+=cspan;}
				}
				
				sourceWidth=jQuery(this).width();
				targetWidth=jQuery(tHTDS[i]).width();
				maxWidth=Math.max(sourceWidth,targetWidth);
				...
				if(isScrollFoot && colspan){
					...
					allWidth+=maxWidth;
					if(cspan==currentCol){
						tFTDSWidth=jQuery(tFTDS[i-allCSpan+1]).width();
						if(tFTDSWidth>allWidth){
							maxWidth=tFTDSWidth-allWidth+maxWidth;
							allWidth=tFTDSWidth;
							jQuery(this).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
							jQuery(tHTDS[i]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
							jQuery(tFTDS[i-allCSpan+1]).css({"width":(allWidth+1*cspan-1)+"px","min-width":(allWidth+1*cspan-1)+"px"});
						}
						else{
							jQuery(this).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
							jQuery(tHTDS[i]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
				...
				}else if(isScrollFoot){
					tFTDSWidth=jQuery(tFTDS[i-allCSpan+1]).width();
					maxWidth=Math.max(sourceWidth,targetWidth,tFTDSWidth);
					jQuery(this).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
					jQuery(tHTDS[i]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
					jQuery(tFTDS[i-allCSpan+1]).css({"width":maxWidth+"px","min-width":maxWidth+"px"});
				}else{
					jQuery(this).css({"min-width":maxWidth+"px"});
					jQuery(tHTDS[i]).css({"min-width":maxWidth+"px"});
				}
			});
			},
			1
	...
	navigate:function(tn,tgt){
		...
		new Ajax.Request(url, {
		  ...
		  onSuccess: function(transport) {
			...
			jQuery("#index_i").removeAttr("style");
		  },
...
/**
	...
	*/
	 ...
	 * order by column, will toggle asc and desc, else do asc
	 	@event
	 */
	orderGrid2: function(td,clinkid,event){
		if(this._checkDirty()==false){
			if (!event) event = window.event;
			if(this._gridQuery.orders==null)this._gridQuery.orders=[];
...
//       	*/
   ...
   if(jQuery("#portal-menu").css("display")=="block" ){
		...
		/*paco add begin*/
		var maxHeight=jQury("#embed-lines").css("max-height");
...
// jQuery(document).ready(function(){
	// mufavorite.main;
	// jQuery("#embed-lines #inc_table tbody tr:first td").bind("resize",function(){
		// alert("td resize");
	// });
// });
