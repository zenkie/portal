(function () {
    ...
    P.Submit = function () {
        ...
        var validContext = { hasError: false };
        masterobj = $.extend(masterobj, magazine);
        if (!this.SubmitValidate2(masterobj)) { return; }
        data.detailobjs.refobjs = [this.GetCurrentMagazineImages(validContext), this.GetCurrentMagazineAnchors(validContext)];
        delete masterobj.ID;
        masterobj.id = id;
        if (validContext.hasError == true) { return; }
    ...
    //获取当前所有过场页
    P.GetCurrentMagazineImages = function (validContext) {
        var mmobj = {
        ...
        pages.each(function () {
            ...
            if (page.PHOTO == "") {
                var item = $(this);
                validContext.hasError = true;
                self.ShowImagePanel();
                self.ShowMessage("过场页图片必须设置", true);
                self.ShowPageError(item, "过场图片必须设置");
        ...
        if (mmobj.modifyList.length <= 0) {
            delete mmobj.modifyList;
        }
        return mmobj;
    }
    ...
    P.GetCurrentMagazineAnchors = function () {
        ...
        if (anchorbj.modifyList.length <= 0) {
            delete anchorbj.modifyList;
        }
        return anchorbj;
    }
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                if (r.code == 0) {
                    ...
                    self.ShowMessage("异常,原因:" + r.message);
                }
            }, error: function () {
                self.ShowMessage("访问出错,请检查是否为网络异常");
                //alert('访问出错');
            }
        });
    }
    ...
    P.InitHandlers = function () {
        ...
        //上传音乐
        this.RegistUploadifys($(".choose-music"), this.GetCall(this.OnSetBGMusic), true, true);
        //背景颜色设置
    ...
    //注册上传图片功能
    P.RegistUploadifys = function (target, success, noWrapper, isAudio) {
        this.UploadifyIdRender(target);
        var self = this;
        target.each(function () { self.RegistUploadify(this, success, noWrapper, isAudio); });
    }

    //注册单个元素上传功能
    P.RegistUploadify = function (item, success, noWrapper, isAudio) {
        var target = $(item);
        var id = target.attr("id");
        var up = this.GetUploadifyParams(id, isAudio);
        var self = this;
        _upinit = up.upinit;
        _para = up.para;
		target.uploadifive({
    ...
    //获取图片上传参数
    P.GetUploadifyParams = function (id, isAudio) {
        var upinit = {
    ...
    P.DeletePage = function (ev) {
        ...
        this.RemoveLinksByPage(page.attr("id"));
        this.AutoSelect(page);
        page.remove();
    }

    //自动选择当前页前一页或者后一页
    P.AutoSelect = function (page) {
        var next = page.next(".js-pages-region:eq(0)");
        var before = page.prev(".js-pages-region:eq(0)");
        if (next.length > 0) {
            this.SelectPage(next);
        } else if (before.length > 0) {
            this.SelectPage(before);
        } else {
            this.SetCurrentDesignImage("");
        }
        this.SyncDesignerImage();
    }

    //删除指过场页下的所有锚点 id:执行过场页的元素id
    P.RemoveLinksByPage = function (id) {
        var links = $(".map-link[data-target=" + id + "]");
        var dataId = 0;
        var link = null;
        for (var i = 0, k = links.length; i < k; i++) {
            link = $(links[i]);
            dataId = link.find("input[name=id]").val();
            if (!isNaN(dataId) && dataId > 0) {
                this.anchorRemoves.push(dataId);
            }
            link.remove();
        }
    }
    ...
    P.AddLink = function () {
        ...
        var photo = page.find("input[name=photo]").val();
        if (photo == '') {
            this.ShowPageError(page, "设置链接前，请先设置图片");
            return;
        }
        if (mapLinks.length > 4) {
            this.ShowPageError(page, "过场页最多只能添加5个链接");
            return;
        }
        var href = '';
        var htmls = [
    ...
    P.ShowPageError = function (page, message) {
        var errorHolder = page.find("label.error");
        if (errorHolder.length <= 0) {
            var el = page.find("a.add-link");
            errorHolder = $('<label class="error" style="border:0px;background:none;"></label>');
            errorHolder.insertAfter(el);
        }
        setTimeout(function () { errorHolder.html(""); }, 2000);
    ...
    //渲染上传元素编号
    P.UploadifyIdRender = function (target, isAudio) {
        var self = this;
