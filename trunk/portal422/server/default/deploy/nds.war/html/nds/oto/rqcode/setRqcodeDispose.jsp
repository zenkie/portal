...
<%
	...
	String tableId="WX_RQCODEDISPOSE";
	...
	int fromid=Tools.getInt(request.getParameter("fromid"),0);
	...
	org.json.JSONObject rqcodeDispose=new org.json.JSONObject();
	org.json.JSONObject jc;
	options.append("<option value=\"\">==请选择活动==</option>");
	for (int i=0;i<result.getRowCount();i++){
		result.next();
		jc=new org.json.JSONObject(); 
		jc.put("ID",result.getObject(1));
		jname =  String.valueOf(result.getObject(2));
		jc.put("NAME",jname);
		...
		//FIFTLE_CONDITION
		jc.put("FIFTLE_CONDITION",result.getObject(7));
		options.append("<option value=\"");
		options.append(result.getString(1)).append("\"");
		if(fromid==Tools.getInt(String.valueOf(result.getObject(1)),0)){
		...
		rqcodeDispose.put(String.valueOf(result.getObject(1)),jc);
	}
%>
...
<script language="javascript" src="/html/nds/oto/rqcode/js/rqcodedispose.js"></script>
<!--
...
function submitMessage(){
	...
	if(chooseUrl.length>=0){
		jQuery("#currentdispose").contents().find("input[name=chooseUrl]:checked").each(function () {
			...
			<table class="fieldTable">
				<tbody>
					<tr>
						<th></th>
						<td>
							<input type="button" value="保存" id="btn" class="btnGreenS" onclick="rqd.submitMessage()">
							<input type="button" value="取消" id="btn" class="btnGreenS" onclick="rqd.cancelMessage()">
						</td>
					</tr>
					<tr>              
						<th>
						...
						<td>
							<select id="alldispose" name="alldispose" onchange="rqd.changeDispose()">
								<%=options%>
							</select>
						</td>
					</tr>
					<tr>
						<th></th>
						<td><div id="currentdispose" style="border:none;"></div></td>
					</tr>
				</tbody>
			</table>
        </form>
