...
keywords.prototype={
	initialize:function(){
		...
		window.onload=function(){
			...
			jQuery('#txtReplyWords').bind({ 'mouseup' : function getJiaodian(v){
				...
				jQuery("#spnCnt").text(editKeyWords.mbStringLength());
	...
	ShowAddKeyword:function(obj){
		...
		s += '<br/><span class="red" id="spnWinErr"></span>';
		s += '</p>';
	...
	AddMenuMark:function(){
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				editKeyWords.insertText("#txtReplyWords",s);
			}
		});
	...
	insertText:function(textarea, str){
		if(jQuery.isEmptyObject(range)){
			...
			jQuery("#spnCnt").text(editKeyWords.mbStringLength(editKeyWords.mbStringLength()));
			return;
		}
		...
		jQuery("#spnCnt").text(editKeyWords.mbStringLength(jQuery("#txtReplyWords").val()));
