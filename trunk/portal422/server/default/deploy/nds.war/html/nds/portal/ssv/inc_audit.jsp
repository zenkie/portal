...
<a href="javascript:void(0);" onclick="javascript:oa.submitAuditForm('reject')" class="button"><%= LanguageUtil.get(pageContext, "object.reject",null)%></a>
...
<span id="user" onaction="<%=fkQueryModel.getButtonClickEventScript()%>">
	...
	<!--<img border=0 width=16 height=16 align=absmiddle src='<%=fkQueryModel.getImageURL()%>' alt='<%= PortletUtils.getMessage(pageContext, "open-new-page-to-search" ,null)%>'>-->
</span>
<script>
...
<div id="order_table_head" class="Barcode-print-wrap">
<table cellspacing="0"  rowh ighlightclass="ewTableHighlightRow" rowselectclass="ewTableSelectRow" roweditclass="ewTableEditRow" class="ewTable index_table" style="margin-top: -2px; background-color: #fff; border-collapse: separate; width: 100%;" id="audit_table">
<thead><tr class="ewTableHeader">
<td width=1 class="left-none-border"><input class='cbx' type="checkbox" id="chk_select_all" name="chk_select_all" value=1 onclick='javscript:pc.checkAll("form1",<%=pageRecordCount%>)'> </td>
...
<td class="right-none-border">延期时间</td>
</tr>
</thead>
<%
...
%>
		<tr class="row" style="cursor: pointer" height="25">
		    <td width="50" class="left-none-border">
			...
			<td class="right-none-border"><%=(System.currentTimeMillis()-crdate.getTime())/(24*60*60*1000)>0?(System.currentTimeMillis()-crdate.getTime())/(24*60*60*1000):0%>天</td>
		</tr>
...
<script type="text/javascript">
		jQuery(window).ready(function() {
            setScroll();
            jQuery(window).unbind('resize',setScroll).bind('resize',setScroll);
        });
        function setScroll(){
            var width=0;
