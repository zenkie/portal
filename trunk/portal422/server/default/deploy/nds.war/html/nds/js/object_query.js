...
ObjectQuery.prototype = {
    ...
    */
        ...
        if (l == null || (l != null && l.title == "popup")) {
            ...
            var options = $H({ animation: animation, queryindex: this._queryindex, opacity: 0.1, background: '#fff', padding: 0, width: 610, height: 200, top: '7%', title: gMessageHolder.SEARCH, skin: 'chrome', drag: true, lock: true, esc: true, effect: false, close: function () { oq.close_query(); } });
            //
    ...
    toggle_m: function (url, accepter_id, options) {
        ...
        if (l == null || (l != null && l.title == "popup")) {
            ...
            this._accepter_id[this._queryindex] = accepter_id;
            var animation = { inCss: 'translateFromTop', outCss: 'translateToBottom' };
            var options = $H({ animation: animation, queryindex: this._queryindex, opacity: 0.1, background: '#fff', padding: 0, width: 610, height: 200, top: '7%', border: true, resize: true, title: gMessageHolder.SEARCH, skin: 'chrome', drag: true, lock: true, esc: true, close: function () { oq.close_query(); } });
            var par = art.dialog(options);
	...
	*/
    ...
    close: function () {
        ...
        for (var i in list) {
            ...
            var cfg = list[i].config || {};
            if (now_popw == title_id && cfg.id != 'turboscan_div') {
                list[i].close();
            }
        };
    },
	...
	*/
    _onQueryObject: function (e) {
        ...
        commjs_registCustomerScrollBar("#query-result");
    },
    ...
    */
    _initGridSelectionControl: function () {
        ...
        var tb = jQuery('#q_grid_table_' + this._queryindex).selectable({
            filter: 'tr', tolerance: "touch", cancel: "input,:input,a,.ui-selected",
...
DynamicQuery.prototype = {
    ...
    toggle: function (text_focus_index) {
        ...
        if (newvalue != "" && newvalue != oldvalue && !flag) {
            ...
            if ($("div_dyn") != null) {
                ...
                $("div_dyn").innerHTML = "<div id='divcontent_dyn' class='divcontent_dyn'></div>";
            } else {
                var dynqueryDiv = document.createElement("div");
                dynqueryDiv.id = "div_dyn";
                dynqueryDiv.style.display = "";
                dynqueryDiv.className = "div_dyn";
                dynqueryDiv.innerHTML = "<div id='divcontent_dyn' class='divcontent_dyn'></div>";
    ...
    */
    setdynamicdata: function () {
        ...
        td.innerHTML = "<a class='dcq_close' href='javascript:void(0)'onclick='dcq.closediv();'>" + gMessageHolder.CLOSE_DIALOG + "</a>";
        var offsets = getOffsets(this._accepter_id);
